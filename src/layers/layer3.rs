...
fn test_correct_parity() {
    assert!(correct_parity(0b10110010));
    assert!(correct_parity(0b00000000));
    assert!(correct_parity(0b11111111));
}

fn combine(bytes: &[u8]) -> Result<Vec<u8>> {
    let good_bytes: Vec<u8> = bytes.into_iter().copied().filter(|&b| correct_parity(b)).collect();

    ensure!(good_bytes.len() % 8 == 0, Error::new(InvalidInput, "input needs to be a multiple of 8 bytes"));

    Ok(good_bytes.as_slice().chunks(8).map(|bytes| {
        // drop parity bit
        ...
        for i in 0..8 {
            let shift = 7 * (7-i);
            temp += ((bytes[i] as u64) << shift) as u64;
        }
        
        let mut out : Vec<u8> = Vec::new();
        // read off 7 bytes
        for _ in 0..7 {
            out.insert(0, (temp & 0b11111111) as u8); //todo this feels bad
            temp >>= 8;
        }
